...
const AddPetForm = () => {
  ...
  const onClick = async e => {
    ...
    if (btn.includes('Next')) {
      ...
      allFields = ['addTitle', 'name', 'birth', 'type'];
      data.option === 'pet' && allFields.shift('addTitle');

      let notCompletedFields = allFields.filter(
        key => !fullFields.includes(key)
      );
    ...
    <form className={styles.form} onClick={onClick}>
      ...
      <div className={btnStyle.buttonsContainer}>
        ...
        >
          ...
          <PawprintIcon
            className={btnStyle.btnLearnIcon}
            width={24}
            height={24}
          />
        </button>
